function readRowAndSpeak() {
  ...
  dataRange.forEach(row => {
    const sku = (row[colIndex["SKU"]] || "").toString().toUpperCase().trim();
    const comboSkuRaw = row[colIndex["Combo SKU"]] || "";
    ...
    const segment1Parts = [];
    if (status !== "") segment1Parts.push(status);
    if (mergeOrder !== "") segment1Parts.push(" " + mergeOrder);
    if (component !== "") segment1Parts.push(" " + component);
      ...
      <div style="text-align: center; font-family: sans-serif; margin-bottom: 40px;">
        <div style="display: flex; justify-content: center; gap: 30px; align-items: center;">
          ...
          <div style="flex: 1; font-size: 14px; text-align: left;">
            ...
            <div style="margin-top: 12px; font-size: 20px; color: #ff0000;">
  ...
  <body>
    ...
    <script>
      ...
      function updateComboImages() {
        ...
        var imgs = (displays[index].comboImages || []).slice(0, 10);
          ...
          case "respeak":
            ...
            if (paused || !synth.speaking) {
             if (toSpeak.length === 0 && held.length === 0) {
               ...
               speakLine([speakTexts[index][2]], function() {
               hideLoading();
             });
           } else {
             hideLoading();
        ...
        recognition.onresult = function(event) {
          for (var i = event.resultIndex; i < event.results.length; i++) {
            if (event.results[i].isFinal) {
              ...
              } else if (spokenText.includes("restart")) {
                showLoading("restart");
...
//     "SKU", "Name", "Quantity", "Post Code", "Selling Platform",
//     "Instruction QR", "Image URLs", "Title", "Price", "Customer Info", 
//     "Combo SKU", "Status", "Merge Order", "Component"
//   ];
...
//      if (name !== "") {
//       const nameWords = name.split(" "); // Split name by spaces
//       segment1Parts.push(":: " + nameWords.join(" ")); // Join with pauses
//     }
...
//     if (name !== "") {
//       const nameWords = name.split(" "); // Split name by spaces
//       segment2Parts.push(" " + nameWords.join(" ")); // Join with pauses
//     }
//     if (quantity !== "") segment2Parts.push(" :: " + quantity + ":  ");
...
//                 return;
//              }
//              // speakTexts[index][1] = Postcode + QR
//                speakLine([speakTexts[index][2]], function() {
//                hideLoading();
//              });
//            } else {
